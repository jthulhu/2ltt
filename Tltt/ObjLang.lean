...
def up.{i} (α : U.{i}) : Type i := El α.intoU
...
namespace B
  ...
  def elim {P : ↑B → El Ty} (b : ↑B) (t : ↑(P true')) (f : ↑(P false')) : ↑(P b) :=
    match b with
    | ⟨true⟩ => t
    | ⟨false⟩ => f
end B
...
namespace Pi
  def lam {α : U} {β : ↑α → U} (f : (a : ↑α) → ↑(β a)) : ↑(Pi α β) :=
    El.mk f

  def app {α : U} {β : ↑α → U} (f : ↑(Pi α β)) (a : ↑α) : ↑(β a) :=
    f.intoU a
...
namespace Id
  ...
  def symm.{i} {α : U.{i}} (x y : ↑α) (p : ↑(x =∘ y)) : ↑(y =∘ x) := by
    apply El.mk
    apply InnerId.symm
    exact p.intoU

  def inv {α : U} {x y : ↑α} : ↑((x =∘ y) →∘ (y =∘ x)) :=
    Λ p => symm x y p

  def trans.{i} {α : U.{i}} (x y z : ↑α) (p₁ : ↑(x =∘ y)) (p₂ : ↑(y =∘ z)) : ↑(x =∘ z) := by
    apply El.mk
